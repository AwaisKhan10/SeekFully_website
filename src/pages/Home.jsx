...
const Home = () => {
 ...
 <Link to="/login"> <button className="px-4 py-1 bg-red-600 text-white text-sm sm:text-base rounded-full">
      Login
    </button>
    </Link>  
 <Link to="/signup"> <button className="px-4 py-1 border border-red-500 text-red-600 text-sm sm:text-base rounded-full">
      Register
    </button>
    </Link>  
  </div>
  ...
  <nav className="flex flex-col items-center mt-8 sm:mt-12">
    ...
    <Link to="/"><li className="text-red-600 border-b-2 border-red-600 pb-1 cursor-pointer">
        Mapping
      </li> </Link>  
   <Link to="/Comunity"><li className="cursor-pointer hover:text-red-600 transition">Community</li> </Link>   
    </ul>
  </nav>
  ...
  <div className="grid gap-4 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
    {templateImages.map((src, idx) => {
      ...
      return (
        <div
  key={idx}
  className={`border-8 ${borderColors[idx % borderColors.length]} rounded-lg overflow-hidden shadow hover:shadow-md transition-shadow duration-300 bg-white`}
>
  {/* Header */}
  <div
    className={`text-white text-center py-2 text-sm font-semibold ${headerBgColors[idx % headerBgColors.length]}`}
  >
