...
>
        ...
        <header className="relative px-4 py-6 sm:px-8">
          ...
          <div className="absolute top-6 right-6 flex space-x-3">
            <Link to="/login">
              <button className="px-5 py-2 w-[120px] h-[40px] bg-red-700 text-white text-md rounded-full shadow hover:bg-red-700">Login</button>
            </Link>
            <Link to="/signup">
              <button className="px-5 py-2 w-[120px] h-[40px] border border-red-800 text-red-700 text-sm  rounded-full hover:bg-red-50">Register</button>
            </Link>
          </div>
          ...
          <nav className="flex justify-center mt-8">
            <ul className="flex space-x-6  font-semibold text-lg">
              ...
              <Link to="/Comunity">
                <li className="text-gray-400 hover:text-red-600 cursor-pointer mt-1">Community</li>
              </Link>
            </ul>
          </nav>
          ...
          <div className="grid gap-6 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {templateImages.map((src, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className={`border-8 ${borderColors[idx % borderColors.length]} rounded-lg overflow-hidden shadow-md bg-white`}
                >
                  <div className={`text-white text-center py-2 text-sm font-semibold ${headerBgColors[idx % headerBgColors.length]}`}>
